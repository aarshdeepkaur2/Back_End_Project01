...
describe('calculatePortfolioPerformance', () => {
    ...
    test('Slight loss (-10% <= change < 0%)', () => {
        ...
        expect(result.profitOrLoss).toBe(-50);
        expect(result.percentageChange).toBeCloseTo(-5);
        expect(result.performanceSummary).toBe('The portfolio has lost slightly with a loss of $50.00.');
    });
    test('Moderate loss (-20% <= change < -10%)', () => {
        ...
        expect(result.performanceSummary).toBe('The portfolio has lost moderately with a loss of $200.00.');
    });
